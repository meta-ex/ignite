...
(def switch-kb? (atom false))


(on-event [:midi-device "KORG INC." "KEYBOARD" "nanoKEY2 KEYBOARD" 1]
          (fn [m]
            (let [kb-id @switch-kb? ]
              (event [:keyboard kb-id (:command m)] m)
              (event [:keyboard kb-id] m)))
          ::kb-one)

(on-event [:midi-device "KORG INC." "KEYBOARD" "nanoKEY2 KEYBOARD" 0]
          (fn [m]
            (let [kb-id (not @switch-kb?) ]
              (event [:keyboard kb-id (:command m)] m)
              (event [:keyboard kb-id] m)))
          ::kb-zero)
...
(defn sam-kb [& args]
  (concat [:keyboard true] args))

(defn jon-kb [& args]
  (concat [:keyboard false] args))
